...
function todayView(){
  ...
  if(d!==5) cards.push({
    cls:d===0?'home-backward':'home-cardio', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', img:d===0?'assets/images/incline-walk-backward.png':'assets/images/incline-walk-forward.png',
        ...
        ${cards.map(c=>`
          <button class="home-training-card ${c.cls}" onclick="${c.action}">
            ...
            <div class="home-card-copy">
              <span>${c.meta} ${completedForToday(c.key||'')?'✅ Completed':''}</span>
...
function progressView(){
  ...
  const best = ex => {
    const vals=db.exerciseLogs.filter(x=>String(x.exercise||'').toLowerCase().includes(ex.toLowerCase())).map(x=>x.working);
    return vals.length?Math.max(...vals):'—';
  };
  screen.innerHTML =
    `${card('Weight Trend', `<div class="big-metric">${ws.cur?ws.cur.weight+' kg':'—'}</div><p class="small">7-day average: ${ws.avg} kg</p><canvas id="weightChart" width="420" height="180"></canvas>`)}
    ...
    ${card('Best Sets', `<div class="list">${['Push-ups','Pull-ups','Chin-ups','Dips','Reverse rows','Dead / active hang','Captain chair knee raises','Crunches'].map(x=>`<div class="list-item"><span>${x}</span><b>${best(x)}</b></div>`).join('')}</div>`)}`;
  drawWeightChart();
}
...
// Phase 20 completion lock/edit system
let editOverrideRoutine = null;

function hasExerciseToday(names){
  const date = todayISO();
  return names.some(n => db.exerciseLogs.some(x =>
    x.date === date && String(x.exercise||'').toLowerCase().includes(n.toLowerCase())
  ));
}
function completedForToday(key){
  const date = todayISO();
  ...
  if(key === 'morningAbs'){
    return hasExerciseToday(['Crunches','Stomach vacuum','Hollow hold']);
  }
  if(key === 'sundayCore'){
    return hasExerciseToday(['Crunches','Reverse crunch','Stomach vacuum','Hollow hold','Dead bug','Plank']);
  }
  if(key === 'cardio-forward'){
    return db.cardioLogs.some(x=>x.date===date && x.type !== 'Backward treadmill walk');
  }
  if(key === 'cardio-backward'){
    return db.cardioLogs.some(x=>x.date===date && x.type === 'Backward treadmill walk');
  }
  if(key === 'pullup' || key === 'chinup' || key === 'hang'){
    return db.exerciseLogs.some(x=>x.date===date && x.routine===key);
  }
  return db.exerciseLogs.some(x=>x.date===date && x.routine===key);
}
...
function todayRoutineInfo(key){
  const date = todayISO();
  if(key === 'morningAbs') return db.exerciseLogs.filter(x=>x.date===date && ['Crunches','Stomach vacuum','Hollow hold'].some(n=>String(x.exercise||'').toLowerCase().includes(n.toLowerCase())));
  if(key === 'sundayCore') return db.exerciseLogs.filter(x=>x.date===date && ['Crunches','Reverse crunch','Stomach vacuum','Hollow hold','Dead bug','Plank'].some(n=>String(x.exercise||'').toLowerCase().includes(n.toLowerCase())));
  if(key === 'cardio-forward') return db.cardioLogs.filter(x=>x.date===date && x.type !== 'Backward treadmill walk');
  if(key === 'cardio-backward') return db.cardioLogs.filter(x=>x.date===date && x.type === 'Backward treadmill walk');
  return db.exerciseLogs.filter(x=>x.date===date && x.routine===key);
}
function openCompletedCard(key,title,goFn){
  const logs = todayRoutineInfo(key);
  const summary = logs.length ? logs.map(x=>x.exercise?`${x.exercise}: ${x.working} ${x.unit}`:`${x.type}: ${x.duration} min`).join('<br>') : 'Completed today.';
  screen.innerHTML = card('✓ Completed Today', `
    <p class="small">${title}</p>
    <div class="completed-summary">${summary}</div>
    <button class="btn primary" id="editCompleted">Edit today</button>
    <button class="btn" id="backHome">Back home</button>
  `);
  document.getElementById('editCompleted').onclick = () => {
    editOverrideRoutine = key;
    goFn();
  };
  document.getElementById('backHome').onclick = () => homeView();
}

function removeTodayRoutineLogs(key){
  const date = todayISO();
  if(key === 'morningAbs'){
    db.exerciseLogs = db.exerciseLogs.filter(x => !(x.date===date && ['Crunches','Stomach vacuum','Hollow hold'].some(n=>String(x.exercise||'').toLowerCase().includes(n.toLowerCase()))));
  }else if(key === 'sundayCore'){
    db.exerciseLogs = db.exerciseLogs.filter(x => !(x.date===date && ['Crunches','Reverse crunch','Stomach vacuum','Hollow hold','Dead bug','Plank'].some(n=>String(x.exercise||'').toLowerCase().includes(n.toLowerCase()))));
  }else if(key === 'cardio-forward'){
    db.cardioLogs = db.cardioLogs.filter(x => !(x.date===date && x.type !== 'Backward treadmill walk'));
  }else if(key === 'cardio-backward'){
    db.cardioLogs = db.cardioLogs.filter(x => !(x.date===date && x.type === 'Backward treadmill walk'));
  }else{
...
// Phase 21 final save/home/completion override
function p21ExerciseNamesForKey(key){
  if(key === 'morningAbs') return ['Crunches','Stomach vacuum','Hollow hold'];
  if(key === 'sundayCore') return ['Crunches','Reverse crunch','Stomach vacuum','Hollow hold','Dead bug','Plank'];
  return [];
}
function p21HasExercisesToday(names){
  const date = todayISO();
  return names.some(n => db.exerciseLogs.some(x =>
    x.date === date && String(x.exercise || '').toLowerCase().includes(n.toLowerCase())
  ));
...
window.completedForToday = function(key){
  const date = todayISO();
  if(key === 'morningAbs') return p21HasExercisesToday(p21ExerciseNamesForKey('morningAbs'));
  if(key === 'sundayCore') return p21HasExercisesToday(p21ExerciseNamesForKey('sundayCore'));
  if(key === 'cardio-forward') return db.cardioLogs.some(x=>x.date===date && x.type !== 'Backward treadmill walk');
  if(key === 'cardio-backward') return db.cardioLogs.some(x=>x.date===date && x.type === 'Backward treadmill walk');
  return db.exerciseLogs.some(x=>x.date===date && x.routine===key);
};
...
function p21RemoveToday(key){
  const date = todayISO();
  const names = p21ExerciseNamesForKey(key);
  if(names.length){
    db.exerciseLogs = db.exerciseLogs.filter(x => !(x.date === date && names.some(n => String(x.exercise||'').toLowerCase().includes(n.toLowerCase()))));
    return;
  }
  if(key === 'cardio-forward'){
    db.cardioLogs = db.cardioLogs.filter(x=>!(x.date===date && x.type !== 'Backward treadmill walk'));
...
window.saveWorkout = function(key,title){
  const date = todayISO();
  // Replace today's entry for this routine so it does not duplicate.
  p21RemoveToday(key);
...
window.saveCardioAndHome = function(payload){
  db.cardioLogs.push(payload);
  save();
  homeView();
  setActive('home');
};
