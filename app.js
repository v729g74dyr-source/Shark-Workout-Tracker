...
function todayView(){
  ...
  if(morningAbsAllowed()) cards.push({
    cls:'home-abs', title:'Morning Abs', meta:'Morning',
    desc:'Crunches · Stomach vacuum · Hollow hold',
    action:`workoutView('morningAbs','Morning Abs')`
  });
  if(rk==='sundayCore') cards.push({
    cls:'home-core', title:'Sunday Core', meta:'Core only',
    desc:'Crunches · Reverse crunches · Vacuum · Hollow · Dead bug · Side plank',
    action:`workoutView('sundayCore','Sunday Core')`
  });
  else if(rk) cards.push({
    cls:'home-power', title:'Power Tower', meta:'Lunch',
    desc:rt,
    action:`workoutView('${rk}','${rt}')`
  });
  if(d!==5) cards.push({
    cls:d===0?'home-backward':'home-cardio',
