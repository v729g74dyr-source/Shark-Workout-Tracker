...
function todayView(){
  ...
  if(d!==5) cards.push({
    cls:d===0?'home-backward':'home-cardio', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', img:d===0?'assets/images/incline-walk-backward.png':'assets/images/incline-walk-forward.png',
...
window.saveWorkout=function(key,title){
  ...
  save(); progressView(); setActive('progress');
};
...
// Phase 20 save override with edit replacement
window.saveWorkout = function(key,title){
  if(editOverrideRoutine === key) removeTodayRoutineLogs(key);
  const date=todayISO();
  document.querySelectorAll('.workout-card').forEach(c=>{
    const setVals=[...c.querySelectorAll('.set-value')].filter(inp=>!inp.classList.contains('rest')).map(inp=>Number(inp.value||0)).filter(v=>v>0);
    let working=0,warmup=0;
