...
function todayView(){
  ...
  if(d!==5) cards.push({
    cls:d===0?'home-backward':'home-cardio', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', img:d===0?'assets/images/incline-walk-backward.png':'assets/images/incline-walk-forward.png',
...
function exerciseCard(name,unit,i){
  ...
  const setFields = isAbsCore ? `${setStepper('Set 1','set1')}${setStepper('Set 2','set2')}${setStepper('Set 3','set3')}` : `${stepper('Warm-up','warm')}${stepper('Working set','working')}`;
  return `<section class="card workout-card visual-workout-card big-art-card" data-ex="${name}" data-unit="${unit}">
    <div class="exercise-big-image art-${art[1]}" style="background-image:linear-gradient(180deg,rgba(3,20,38,.04),rgba(3,20,38,.78)),url('${exerciseImage(name)}')">
      ...
      <div class="big-image-overlay"><span>${unit}</span><h3>${i+1}. ${name}</h3><p>${prev?`Last: ${prev.working} ${prev.unit} · ${prev.intensity} · rest ${prev.restSec}s`:'Log your working sets.'}</p></div>
    </div>
