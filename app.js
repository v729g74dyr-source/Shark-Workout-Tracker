...
function todayView(){
  ...
  if(d!==5) cards.push({
    cls:d===0?'home-backward':'home-cardio', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', key:d===0?'cardio-backward':'cardio-forward', img:d===0?'assets/images/incline-walk-backward.png':'assets/images/incline-walk-forward.png',
...
function p22RouteForKey(key){
  ...
  if(key === 'morningAbs') return () => workoutView('morningAbs','Morning Abs',morningAbs);
  if(key === 'sundayCore') return () => workoutView('sundayCore','Sunday Core',sundayCore);
  if(key === 'pullup') return () => workoutView('pullup','Pull-up Day',pullupDay);
  if(key === 'chinup') return () => workoutView('chinup','Chin-up Day',chinDay);
  if(key === 'hang') return () => workoutView('hang','Hang + Row Day',hangDay);
